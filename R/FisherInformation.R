...
fi_pool_imperfect <- function(s,p,sensitivity,specificity){
  ...
  #probpos <- (1-p)^s * (1- varphi - psi) + varphi

  #(s * (1-p)^(s -1) * (1- varphi - psi))^2/ (probpos * (1-probpos))

  q <- 1-p
  s^2 * (1 - psi - varphi)^2 /
    (q^(2 - 2*s)*(varphi + q^s * (1 - psi - varphi))*(1 - varphi - q^s*(1 - psi - varphi)))
...
fi_pool_imperfect_cluster <- function(s, N, prevalence, sensitivity, specificity,
  ...
  phi <- function(p){sens + (1 - spec - sens) * (1 - p)^s}
  ...
  if(form == 'discrete'){
    ...
    return(matrix(c(sum(lik_theta^2/lik),
    ...
    Alpha <- theta * rho.;
    Beta <- (1 - theta) * rho.;
    if(sens == 1 & spec == 1 & length(s) == 1 && N < 10){ #Cases where we have solutions in terms of beta functions etc. However this method becomes numerically unstable for large N
      y <- 0:N
      if(s == 1){ #un-pooled case with a perfect test has simple exact solution
        ...
        const <- choose(N,y) / beta(Alpha, Beta)
        var <- sapply(y, function(y){
          x <- 0:y

          # removing the multiplicative constant (see 'const' above)
          lik <- 1 * #choose(N,y) / beta(Alpha, Beta) *
            sum((-1)^x * choose(y,x) * beta(Alpha, Beta + s * (N - y + x)))

          lik_theta <- (Alpha + Beta) * #choose(N,y) / beta(Alpha, Beta) *
            sum((-1)^x * choose(y,x) * beta(Alpha, Beta + s * (N - y + x)) *
                  (digamma(Beta) - digamma(Beta + s * (N - y + x))))

          lik_rho   <- (Alpha + Beta + 1)^2 * #choose(N,y) / beta(Alpha, Beta) *
            sum((-1)^x * choose(y,x) * beta(Alpha, Beta + s * (N - y + x)) *
                  ((digamma(Beta) - digamma(Beta + s * (N - y + x))) * (1 - theta) +
                     digamma(Alpha + Beta + s * (N - y + x)) - digamma(Alpha + Beta)))

          c(lik_theta^2, rep(lik_theta * lik_rho, 2), lik_rho^2)/lik
        })

        return(matrix(colSums(const * t(var)), nrow = 2))
      }
    }else{ # most general case (imperfect test, mix of pool sizes, or large N) requires numerical integration
      ...
      lik_theta <- apply(ys,1,function(y){
        #plot(function(x){integrand_theta(x,y)},main = paste('integrand_theta',y), n = 10000)
        (integrate(integrand_theta,0,1,y = y, rel.tol = tol, abs.tol = tol)$value + lik_theta_correction(y)) *
          prod(choose(N,y))
      })

      lik_rho <- apply(ys,1,function(y){
        #plot(function(x){integrand_rho(x,y)},main = paste('integrand_rho',y, Alpha, Beta), n = 10000)
      ...
      if(abs(sum(lik) - 1)   > tol ||
         ...
         abs(sum(lik_rho))   > tol){
        print(lik); print(sum(lik)-1); print(lik_theta); print(sum(lik_theta)); print(lik_rho); print(sum(lik_rho))
        print(s);print(N);print(prevalence);print(sensitivity);print(specificity);print(correlation);print(form)
    ...
    link <- switch(form, logitnorm = qlogis, cloglognorm = cloglog)
    ...
    if(abs(sum(lik) - 1)   > tol ||
      ...
      print(lik); print(sum(lik)-1); print(lik_mu); print(sum(lik_mu)); print(lik_sigma); print(sum(lik_sigma))
      print(s);print(N);print(prevalence);print(sensitivity);print(specificity);print(correlation);print(form)
      stop('Error in integration of likelihoods. Likelihoods do not add to 1 or derivatives of likelihood with respect to parameters do not sum to 0')
    }

    FI <- matrix(c(sum(lik_mu^2/lik),
    ...
    if(real.scale){ #If we want FI for the parameters on the real scale {mu,sigma} we're done!
      ...
      J <- solve(matrix(c(dtheta_dmu, dtheta_dsigma,
