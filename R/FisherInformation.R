...
fi_pool_imperfect_cluster <- function(s, N, prevalence, sensitivity, specificity,
  ...
  if(form %in% c('discrete', 'beta')){ # In these cases Fisher information matrix is calculated directly in terms of theta and rho and checks on sums of likelihoods and likelihood derivatives can be performed accordingly
    if(form == 'discrete'){
      method <- 'summation'
      ...
      if(sens == 1 & spec == 1 & length(s) == 1 && (s == 1 | N < 20)){ #Cases where we have solutions in terms of beta functions etc. However this method becomes numerically unstable for large N
        ...
        lik <- apply(ys,1,function(y){
          plot(function(x){integrand(x,y)},main = paste('integrand', y), to = 0.1,n = 10000)
          (integrate(integrand,0,1,y = y, rel.tol = tol, abs.tol = tol)$value + lik_correction(y)) *
            prod(choose(N,y))
        })

        lik_theta <- apply(ys,1,function(y){
          plot(function(x){integrand_theta(x,y)},main = paste('integrand_theta',y), to = 0.1,n = 10000)
          (integrate(integrand_theta,0,1,y = y, rel.tol = tol, abs.tol = tol)$value + lik_theta_correction(y)) *
            prod(choose(N,y))
        })

        lik_rho <- apply(ys,1,function(y){
          plot(function(x){integrand_rho(x,y)},main = paste('integrand_rho',y, Alpha, Beta), to = 0.1,n = 10000)
